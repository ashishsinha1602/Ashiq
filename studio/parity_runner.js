...
const fs = require("fs");
const schemagate = require("./schemagate.js");
...
for (const c of cases) {
  ...
  const cat = cats[key];
  const who = c.principal ? schemagate.principal(c.principal, c.roles) : null;
  ...
  results.push({
    ...
    tokens: schemagate.estimateTokens(sel.promptFragment()),
    ddl_len: sel.promptFragment().length,
  });
}
// tokenizer and embedder spot checks
const tok = {};
for (const t of ["facturación_mensual", "año", "Müller_Straße", "売上明細", "한국어_테이블", "CustOrderLine", "FCT_TXN_LN_2024"]) tok[t] = schemagate.tokenize(t);
const emb = new schemagate.HashingEmbedder(64).embed(["customer order line items", "売上明細 金額"]).map((v) => v.map((x) => Number(x.toFixed(12))));
