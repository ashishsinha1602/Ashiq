"""Command line: try ashiq in thirty seconds without writing any code.

    ashiq demo                                  # bundled schema, no database
    ashiq demo "things we're running out of"    # your own question against it
    ashiq select "revenue by month" --url postgresql://localhost/app
    ashiq select "salary by employee" --url ... --principal okta:jdoe --role finance
    ashiq studio --url postgresql://localhost/app   # the Studio page, locally
    ashiq certify postgresql://user:pw@host/db  # end-to-end check on your engine
...
def cmd_demo(args) -> int:
    ...
    print(f"ashiq {__version__} -- demo schema, {len(cat._docs)} objects, "
    ...
    if not args.question:
        print("Try:  ashiq demo \"things we're running out of\"")
        print("      ashiq demo \"salary by employee\" --principal okta:hr --role payroll")
        print("      ashiq demo --prompt      # the DDL your model would receive")
    return 0
...
def cmd_certify(args) -> int:
    ...
    sys.exit("ashiq: certify_dialect.py is in the source repository:\n"
             "  https://github.com/ashishsinha1602/ashiq/blob/main/scripts/certify_dialect.py")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--version", action="version", version=f"ashiq {__version__}")
